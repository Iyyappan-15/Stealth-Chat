class WebRTCManager {
    constructor(signalingUrl, onMessageReceived, onPeerConnected, onPeerDisconnected, myName) {
        ...
        this._peerConnectedFired = false; // prevents onPeerConnected from firing twice
    ...
    setupDataChannel(channel) {
        ...
        this.dataChannel.onopen = () => {
            ...
            this.p2pConnected = true;
            this._fireOnPeerConnected();
        };

        // If ondatachannel fires after the channel is already open (rare on some browsers)
        if (this.dataChannel.readyState === 'open') {
            console.log('DataChannel already OPEN on setup.');
            this.p2pConnected = true;
            this._fireOnPeerConnected();
        }
    ...
    /** Fires onPeerConnected exactly once per connection. Guard prevents double-fire. */
    _fireOnPeerConnected() {
        if (this._peerConnectedFired) return;
